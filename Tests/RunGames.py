...
from ExactValues.Loop import find_Loop_PDF, find_Loop_PDF_approx
...
def compare_methods_on_grid(m, n, N, solvers, against_exact_Loop_PDF = False):
    ...
    plots score distributions and score-per-apple averages.
    If against_exact_Loop_PDF is True, compares against exact Loop PDF. 
    Unrecommended for large grids,as it is slow to compute.
    """
    ...
    if area % 2 == 0:
        ...
        Loop_mean, Loop_std, approx_Loop_PDF_func = find_Loop_PDF_approx(area)

        # cut off score axis at 3 standard deviations above mean
        ...
        if against_exact_Loop_PDF:
            ...
            Loop_PDF = [approx_Loop_PDF_func(x) for x in score_axis]
